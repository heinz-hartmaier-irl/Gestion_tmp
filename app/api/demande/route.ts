...
export async function POST(req: NextRequest) {
  try {
    ...
    // ✅ On récupère le motif (utilisé pour H.Sup, mais dispo pour tous si besoin)
    const motifText = formData.get("justificatifText") as string | null;

    if (!userId || !startDate || !endDate) return NextResponse.json({ error: "Données incomplètes" }, { status: 400 });
    ...
    let dbType = "";
    switch (typeKey) {
      case "maladie": dbType = "Arrêt Maladie"; break;
      case "conge": dbType = "Congés Payés"; break;
      case "hsup": dbType = "Heures Supplémentaire"; break;
      case "specifique": dbType = "Congé spécifique"; break;
    ...
    try {
      const [users] = await connection.query<RowDataPacket[]>("SELECT solde_conge, solde_hsup FROM user WHERE id_user = ?", [userId]);
      if (users.length === 0) return NextResponse.json({ error: "User inconnu" }, { status: 404 });
      const user = users[0];
      const diffMs = Math.abs(end.getTime() - start.getTime());

      if (dbType === "Congés Payés") {
        const daysRequested = Math.ceil(diffMs / (1000 * 60 * 60 * 24)) || 1;
        if (user.solde_conge < daysRequested) return NextResponse.json({ error: `Solde insuffisant (${user.solde_conge}j).` }, { status: 400 });
      } else if (dbType === "Heures Supplémentaire") {
        const hoursRequested = diffMs / (1000 * 60 * 60);
        if (user.solde_hsup < hoursRequested) return NextResponse.json({ error: `Heures insuffisantes (${user.solde_hsup}h).` }, { status: 400 });
      }
      ...
      // ✅ INSERTION SIMPLIFIÉE AVEC MOTIF
      ...
      // Gestion Fichier (Maladie / Spécifique)
      let dbPath = null;
      ...
      if (typeKey === "maladie") {
        ...
        await connection.execute(`INSERT INTO maladie_spec (id_demande, justificatif) VALUES (?, ?)`, [demandeId, dbPath]);
      } 
      else if (typeKey === "specifique") {
        await connection.execute(`INSERT INTO conges_spec (id_demande, nature, justificatif) VALUES (?, ?, ?)`, [demandeId, nature || "Autre", dbPath]);
      }
      // 🚀 PLUS BESOIN DE INSERT INTO justif_h_supp
