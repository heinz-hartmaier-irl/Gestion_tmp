      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="flex items-center">
            <Link href={isAdmin ? "/profil" : "/profil"} className="flex items-center gap-3 group">
              <div className="relative">
                  ...
                  {user && user.photo ? (
                    <img src={user.photo} alt="Profil" className="relative w-11 h-11 rounded-full object-cover border-2 border-[#000091] bg-white"/>
                  ) : (
          ...
          <button onClick={handleLogout} className="hidden md:block px-6 py-2.5 bg-white border-2 border-[#ff6400] text-[#ff6400] rounded-full text-sm font-bold hover:bg-[#ff6400] hover:text-white transition duration-300 shadow-sm hover:shadow-orange-200">
             Sortir
          </button>

          {/* BOUTON MOBILE */}
          <div className="flex items-center md:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="text-[#000091] p-2 hover:bg-gray-50 rounded-xl transition">
        ...
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-gray-100 absolute w-full left-0 shadow-2xl rounded-b-[2rem] overflow-hidden transition-all animate-fadeIn">
          <div className="px-6 py-6 space-y-3">
            ...
            <div className="border-t border-gray-100 my-2 pt-4">
              <button onClick={handleLogout} className="w-full text-left block px-4 py-3 rounded-2xl text-lg font-[Modak] text-[#ff6400] hover:bg-orange-50 transition-colors">
                Déconnexion
              </button>
            </div>
          </div>
